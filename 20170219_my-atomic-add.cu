...
int main(int argc, char **argv)
{
    int h_sharedInteger;
    int *d_sharedInteger;
    CHECK_CUDA(cudaMalloc((void **)&d_sharedInteger, sizeof(int)));
    CHECK_CUDA(cudaMemset(d_sharedInteger, 0x00, sizeof(int)));

    kernel<<<4, 128>>>(d_sharedInteger);

    CHECK_CUDA(cudaMemcpy(&h_sharedInteger, d_sharedInteger, sizeof(int),
